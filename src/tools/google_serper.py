...
class GoogleSerperAPIWrapper(BaseModel):
    ...
    def mini_batch_run(self, query: str, **kwargs: Any) -> str:
        """Run mini-batch query through GoogleSearch and parse result."""
        results = self._google_serper_api_results(
            query,
            gl=self.gl,
            hl=self.hl,
        ...
        for res in results:
            try:
                parse_res = self._parse_results(res)
            except Exception as e:
                print('Error!! ', e)
                parse_res = 'No good Google Search Result was found'
    ...
    def _google_serper_api_results(
        self, search_term: str or list, search_type: str = "search", **kwargs: Any
    ) -> dict:
        headers = {
        ...
        if isinstance(search_term, str):
            ...
            responses = requests.post(
                f"https://google.serper.dev/{search_type}", headers=headers, data=batch_params
            )
            responses.raise_for_status()
            search_results = responses.json()
